...
export default function EditTaskPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch task data and projects in parallel
        ...
        setErrors(prev => ({ ...prev, fetch: 'Failed to load task data' }));
      } finally {
        setIsLoading(false);
      }
    };
    ...
    try {
      const res = await fetch('/api/tasks/validate-name', {
        ...
        body: JSON.stringify({
          ...
          excludeId: taskId, // Exclude current task from duplicate check
        }),
      });
  ...
  const validateForm = async() => {
    ...
    // Check for duplicate title only if title or project changed
    if (formData.title !== originalTask?.title || formData.projectId !== originalTask?.projectId) {
